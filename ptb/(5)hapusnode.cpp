...
void deletePtbNode(PtbNode *&root, int target) {
    if (root == nullptr) // berarti belum ada datanya
        return;
    cout << root->data << endl;
    cout << root->right << endl;

    if (target < root->data) {
        deletePtbNode(root->left, target);
    } else if (target > root->data) {
        deletePtbNode(root->right, target);
    } else {
        PtbNode *toDelete = root;
        if (root->left == nullptr) {
            ...
            deletePtbNode(root->right, successor->data);
            return;
        }
        cout << "\nroot: " << root->data;
        delete toDelete;
